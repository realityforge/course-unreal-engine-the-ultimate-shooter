...
#include "CheckTextureSubTypePartOfName.h"
#include "Editor.h"
...
void UCheckTextureSubTypePartOfName::Apply_Implementation(TScriptInterface<IRuleRangerActionContext>& ActionContext,
                                                          UObject* Object)
{
    if (IsValid(Object))
    {
        if (IsValid(NameConventionsTable))
        {
            ...
            if (!NameConventionsCache.IsEmpty())
            {
                if (const auto Texture = Cast<UTexture2D>(Object); !Texture)
                {
                    UE_LOG(RuleRanger,
                           Warning,
                           TEXT("CheckTextureSubTypePartOfName: Attempt to run on Object %s "
                                "that is not a Texture2D instance."),
                           *Object->GetName());
                }
                else
                {
                    const TArray<ETextureSubType> SubTypes = ExtractSubTypes(Object->GetName());
                    if (SubTypes.IsEmpty())
                    {
                    ...
                    else
                    {
                        ...
                        for (const ETextureSubType SubType : SubTypes)
                        {
                            switch (SubType)
                            {
                                ...
                                default:
                                {
                                    ...
                                    ActionContext->Error(
                                        FText::FormatNamed(NSLOCTEXT("RuleRanger",
                                                                     "UnhandledSubType",
                                                                     "Unable to parse texture subtype {SubType}"),
                                                           TEXT("SubType"),
                        ...
                        else
                        {
                            UE_LOG(RuleRanger,
                                   VeryVerbose,
                                   TEXT("CheckTextureSubTypePartOfName: Subtypes extracted and "
                                        "match the component size of textures."));
                        }
                    }
                }
            }
        ...
        else
        {
            UE_LOG(RuleRanger,
                   Error,
                   TEXT("CheckTextureSubTypePartOfName: Action has not specified "
                        "NameConventionsTable property and will not be applied as a result."));
        }
    }
}

void UCheckTextureSubTypePartOfName::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
    const FName PropertyName = PropertyChangedEvent.Property ? PropertyChangedEvent.Property->GetFName() : NAME_None;
    // ReSharper disable once CppTooWideScopeInitStatement
    const FName TableName = GET_MEMBER_NAME_CHECKED(UCheckTextureSubTypePartOfName, NameConventionsTable);
...
void UCheckTextureSubTypePartOfName::ResetCacheIfTableModified(UObject* Object)
{
...
void UCheckTextureSubTypePartOfName::ResetNameConventionsCache()
{
    UE_LOG(RuleRanger, VeryVerbose, TEXT("TextureSubTypeRenameAction: Resetting the Name Conventions Cache"));
...
void UCheckTextureSubTypePartOfName::RebuildNameConventionsCacheIfNecessary()
{
    if (NameConventionsCache.IsEmpty() && 0 != NameConventionsTable->GetTableData().Num())
    {
        ...
        OnObjectModifiedDelegateHandle = FCoreUObjectDelegates::OnObjectModified.AddUObject(
            this,
            &UCheckTextureSubTypePartOfName::ResetCacheIfTableModified);
        ...
        for (auto NameConventionEntry : NameConventionsCache)
        {
            UE_LOG(RuleRanger,
                   VeryVerbose,
                   TEXT("TextureSubTypeRenameAction: %d conventions in cache"),
                   NameConventionsCache.Num());
        }
    }
}

TArray<ETextureSubType> UCheckTextureSubTypePartOfName::ExtractSubTypes(const FString& Name)
{
